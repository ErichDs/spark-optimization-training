...
object Partitioners {
  ...
  // comment lines 19-55 to execute last RDD: spark hangs for IDK reason
  ...
  val rangedNumbers =
    keyedNumbers.partitionBy(new RangePartitioner(5, keyedNumbers))
  /*
  ...
  def generateRandomWords(nWords: Int, maxLength: Int) = {
    val r = new Random()
    (1 to nWords).map(_ =>
      r.alphanumeric.take(r.nextInt(maxLength)).mkString("")
    )
  }
