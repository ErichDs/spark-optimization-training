...
object SparkRecap {
  ...
  // the entry point of the Spark structured API
...
//    .config("spark.master", "local")
  ...
  val usefulCarsData = cars.select(
    ...
    $"Year", // (interpolator) another column object (needs spark implicits)
  ...
  // low-level API: RDDs (Resilient Distributed Datasets)
  ...
  // RDD -> DF
  val numbersDF = numbersRDD.toDF(
    "number"
  ) // you lose type information, you get SQL capabilities

  // RDD -> DS
  val numbersDS = spark.createDataset(
    numbersRDD
  ) // you get type information + SQL capabilities
