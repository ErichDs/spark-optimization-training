...
object SparkJobAnatomy {
  ...
  // UI: 2 stages, one with 8 tasks, one with 23 tasks
  // each stage is delimited by shuffles

  // copy data to the cluster: docker cp src/main/resources/data/ spark-cluster-spark-master-1:/tmp
  ...
  val avgSalaries =
    empGroups.mapValues(salaries => salaries.map(_.toInt).sum / salaries.size)
  // show the result
  avgSalaries
