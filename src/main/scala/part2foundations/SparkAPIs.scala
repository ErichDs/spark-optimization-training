...
object SparkAPIs {
  ...
  /*
    ...
    AdaptiveSparkPlan isFinalPlan=false
    +- HashAggregate(keys=[], functions=[count(1)])
       +- Exchange SinglePartition, ENSURE_REQUIREMENTS, [plan_id=243]
          +- HashAggregate(keys=[], functions=[partial_count(1)])
             +- SerializeFromObject
                +- Scan[obj#1]
  ...
  dsTimes5Count.show()
  /*
    6 seconds from 0.1 seconds! That's a 70x time increase.
    Let's explain:
    ...
    scala>  dsCount.explain
     ...
     +- HashAggregate(keys=[], functions=[count(1)])
        +- Exchange SinglePartition, ENSURE_REQUIREMENTS, [plan_id=304]
           +- HashAggregate(keys=[], functions=[partial_count(1)])
              +- Project
                 +- Range (1, 1000000000, step=1, splits=8)
   ...
   == Physical Plan ==
    ...
    +- HashAggregate(keys=[], functions=[count(1)])
       +- Exchange SinglePartition, ENSURE_REQUIREMENTS, [plan_id=329]
          +- HashAggregate(keys=[], functions=[partial_count(1)])
             +- SerializeFromObject
                +- MapElements $line39.$read$$iw$$iw$$iw$$iw$$iw$$iw$$iw$$iw$$iw$$iw$$Lambda$4827/1304342922@768707e0, obj#61: bigint
                   +- DeserializeToObject staticinvoke(class java.lang.Long, ObjectType(class java.lang.Long), valueOf, id#23L, true, false, true), obj#60: java.lang.Long
                      +- Range (1, 1000000000, step=1, splits=8)
